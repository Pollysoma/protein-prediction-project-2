...
MAX_SEQ_LENGTH = 1022
DEFAULT_CACHE_DIR = Path(__file__).resolve().parent.parent / "hf_models_nvidia"

# Allow switching between Meta and NVIDIA variants.
ESM2_SIZE_TO_MODEL = {
    "facebook": {
    ...
    "nvidia": {
        "8M": "nvidia/esm2_t6_8M_UR50D",
        "35M": "nvidia/esm2_t12_35M_UR50D",
        "150M": "nvidia/esm2_t30_150M_UR50D",
        "650M": "nvidia/esm2_t33_650M_UR50D",
        "3B": "nvidia/esm2_t36_3B_UR50D",
        "15B": "nvidia/esm2_t48_15B_UR50D",
    },
}


def resolve_model_name(size: str, provider: str) -> str:
    table = ESM2_SIZE_TO_MODEL.get(provider)
    if not table:
        raise SystemExit(f"Unknown provider '{provider}'. Choose from {list(ESM2_SIZE_TO_MODEL)}")
    try:
        return table[size]
    except KeyError:
        raise SystemExit(f"Unknown model size '{size}'. Choose from {list(table)}") from None
...
def main():
    ...
    parser.add_argument(
        "--provider",
        type=str,
        choices=list(ESM2_SIZE_TO_MODEL.keys()),
        default="facebook",
        help="Model provider (Meta/Facebook originals or NVIDIA replicas)",
    )
    parser.add_argument(
        "--cache-dir",
        type=str,
        default=None,
        help=f"Optional HF cache directory (default: {DEFAULT_CACHE_DIR})",
    )
    parser.add_argument(
        "--n",
        type=int,
    ...
    parser.add_argument(
        "--trust-remote-code",
        action="store_true",
        help="Allow loading models that require custom code (needed for NVIDIA ESM2 repos).",
    )
    ...
    logging.info(f"Using HF cache dir: {cache_dir}")
    ...
    model_name = resolve_model_name(args.modelsize, args.provider)

    # Load model/tokenizer once
    logging.info(f"Loading tokenizer and model: {model_name}")
    tokenizer = AutoTokenizer.from_pretrained(
        model_name, cache_dir=str(cache_dir), trust_remote_code=args.trust_remote_code
    )

    if args.flash_attention:
        attn_implementation = "flash_attention_2"
    elif args.provider in {"facebook", "nvidia"}:
        # ESM models in transformers do not yet support SDPA reliably; fall back to eager attention.
